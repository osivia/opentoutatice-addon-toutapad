...
public class EtherpadClientServiceImpl extends DefaultComponent implements EtherpadClientService {
    ...
    // } catch (UnsupportedEncodingException e) {
    // throw new NuxeoException("Failed to obtain the public PAD URL, error: ", e);
    // }
    //
    // return url;
    // }

    private EtherpadClientServiceDescriptor getDescriptor() throws NuxeoException {
        if (StringUtils.isNotBlank(this.serverName)) {
            return this.descriptors.get(this.serverName);
        } else {
            throw new NuxeoException(
                    "No Etherpad server is defined and enabled from extension point '" + EtherpadClientServiceImpl.EXTENSION_POINT_SERVER + "'");
        }
    }

    private void initializeEtherpadClient() throws NuxeoException {
        try {
            this.client = new EPLiteClient(getDescriptor().getPrivateServerURL(), getDescriptor().getApiKey());
        } catch (Exception e) {
            throw new NuxeoException("Failed to instanciate Etherpad client, error: " + e.getMessage());
        }
    }

    private EPLiteClient getClient() throws NuxeoException {
        if (null == this.client) {
            initializeEtherpadClient();
        }
        return this.client;
     ...
     * (non-Javadoc)
     * 
     * @see fr.toutatice.addons.toutapad.ecm.services.EtherpadClientService#grantAccess(org.nuxeo.ecm.core.api.CoreSession,
